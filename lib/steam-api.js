import { enrichWishlistReviews } from '../functions/steam-sync.js';
...
export async function fetchWishlist({ steamId, country = 'ru', language = 'russian', fetchImpl = fetch, concurrency = 4 }) {
  ...
  const games = await enrichWishlistReviews(result, { fetchImpl, country, language });
  return { games, syncedAt: new Date().toISOString() };
}
